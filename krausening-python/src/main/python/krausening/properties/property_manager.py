    ...
    def get_properties(self, file_name: str):
        ...
        if base is not None:
            try:
                ...
                self._logger.warn('No base file found for {0}{1}'.format(base, file_name))
        ...
        if extension is not None:
            try:
                ...
                self._logger.warn('No extension file found for {0}{1}'.format(base, file_name))
...
class EncryptableProperties(Properties):
    ...
    def getProperty(self, key: str, defaultValue: Optional[T] = None):
        ...
        if value is not None:
            if value.startswith(self.__propertyPrefix) and value.endswith(self.__propertySuffix):
